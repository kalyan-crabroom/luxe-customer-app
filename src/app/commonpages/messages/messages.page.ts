...
export class MessagesPage implements OnInit {
  ...
  async getChatUserList(loginUser: any) {
    ...
    this.chatService.getMessages().subscribe((res: any) => {
      ...
      var user_list: any[] = [];
      // var chatList = res.filter((x: { from: any; to: any; }) => x.from == loginUser.id || parseInt(x.to) == loginUser.id);
      var chatList = res.filter((x: { from: any; to: any; }) =>
        x.from == loginUser.id || Number(x.to) == loginUser.id
      );
      console.log("cahtlsit :", chatList)
